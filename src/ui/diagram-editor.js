...
class DiagramEditor extends EventEmitter {
  ...
  constructor (config) {
    ...
    this.canvasElement.onmousedown = (e) => {
      ...
      } else if (self.middleButtonDown) { // viewpoint move (middle button)
        self.setCursor(CURSOR_MOVE)
        self.downX = e.offsetX
        self.downY = e.offsetY
    ...
    this.canvasElement.onmouseup = (e) => {
      ...
      if (e.metaKey || e.ctrlKey) { // viewpoint move
        ...
        self.downX = 0
        self.downY = 0
      } else if (e.button === 1) { // viewpoint move (middle button)
        self.setCursor(CURSOR_DEFAULT)
        self.downX = 0
        self.downY = 0
      } else if (self.activeHandler) {
    ...
    this.canvasElement.onwheel = (e) => {
      const dx = -e.deltaX
      const dy = -e.deltaY
      if (e.ctrlKey || e.metaKey) { // zoom with wheel
        e.preventDefault()
        if (dy < 0) {
      ...
      } else if (e.shiftKey && Math.abs(dx) === 0) { // horizontal scroll (only for non macOS)
